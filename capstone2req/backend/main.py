...
class PipelineRunRequest(BaseModel):
    ...
    skip_done: Optional[bool] = True  # skip insights already processed by a step
    steps: Optional[List[str]] = None  # subset of ["tag", "persona", "index"]
...
# ============ UNIFIED PIPELINE (tag -> persona -> index) ============
# Replaces the old separate "tag batch", "generate all personas" and
# "build index" endpoints with one orchestrated, backgrounded pipeline.

@app.post("/api/pipeline/analyze/{insight_id}")
async def pipeline_analyze_insight(insight_id: str):
    """Run tag + persona generation for one insight in a single call."""
    try:
        result = pipeline.analyze_insight(insight_id)
...
async def pipeline_run(request: PipelineRunRequest = None):
    """Kick off the tag -> persona -> index pipeline as a background job."""
    try:
        limit = request.limit if request else None
        skip_done = request.skip_done if request else True
        steps = request.steps if request else None
        job_id = pipeline.run_pipeline_async(limit=limit, skip_done=skip_done, steps=steps)
...
async def pipeline_get_job(job_id: str):
    """Get status/progress/log for a pipeline job."""
